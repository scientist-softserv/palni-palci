    ...
    module Analytics
      class CollectionReportsController < AnalyticsController
        ...
        def index
          ...
          # Retrieve the collection data from the Google Analytics Reporting API
          all_top_collections = Hyrax::Analytics.top_events('work-in-collection-view', date_range)

          # Filter the collection data to exclude deleted collections
          active_top_collections = all_top_collections.reject do |collection|
            begin
          ...
          @top_collections = paginate(active_top_collections, rows: 10)
