...
# Override from hyrax 3.4.1 to remove #create_default_access_for, which granted deposit access to all registered users
...
module Hyrax
  ...
  class AdminSetCreateService
    ...
    # @return [Hyrax::AdministrativeSet] The fully created admin set.
    # @raise [RuntimeError] if admin set cannot be persisted
    # OVERRIDE Hyrax 3.4.1 adds create_permission_template method
    def create!
    ...
    def workflow_agents
      [
        Hyrax::Group.new({ name: admin_group_name }) # must pass hash as argument
      ].tap do |agent_list|
    ...
    def create_default_access_for(permission_template:, workflow:)
      ...
      workflow.update_responsibilities(role: deposit, agents: Hyrax::Group.new({ name: 'registered' })) # must pass hash as argument
    end
