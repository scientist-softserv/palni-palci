...
RSpec.describe 'Collection reader role can be assigned to Groups and Users', type: :feature, js: true, clean: true do
  context 'a user assigned to a group' do
    ...
    let!(:role) { FactoryBot.create(:role, name: "Collection Reader") }

    it 'receives the privileges of the group' do
      expect(user_ability.collection_reader?).to eq false
      expect(group.members.include?(user)).to eq false
      expect(role.name).to eq "Collection Reader"
    ...
    scenario 'admin assigns a collection editor role to a user' do
      ...
      expect(find("#assigned-role-#{role.id}").find('td:first-child').text).to eq "Collection Reader"
